...
- **Vercel Deployment & Caching**:
    - **Build Cache**: The project uses a custom build script (`scripts/vercel_build.sh`) to significantly speed up deployments. It manually restores cached assets (`webp` images, `lqip` placeholders, and the `public/gallery-data.json` file) from the `.vercel/cache` directory (which Vercel persists between builds) before running the build. After the build, it saves the newly generated assets back to the cache directory. This ensures the generation script only processes new or changed wallpapers, saving significant time.
