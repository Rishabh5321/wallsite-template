...
- **Dynamic Self-Hosting**: A `Dockerfile` and `docker-entrypoint.sh` are provided for easy self-hosting.
    - **Persistent Gallery Cache**: The `docker-compose.yml` is configured to use a named volume (`wallsite_public`) to persist the generated `public` directory on the host machine. The entrypoint script checks for the existence of this cached data and, if found, skips the entire build process, allowing the container to start almost instantly on subsequent runs.
    - **Optimized Runtime**: The `build:assets` script now uses a symbolic link (`ln -s`) instead of copying all wallpapers during the build. This dramatically reduces container startup time and disk space usage, while still allowing original-quality downloads.
    - **Robust Entrypoint**: The `docker-entrypoint.sh` script includes a pre-flight check to ensure the user has correctly mounted their wallpaper directory to `/app/wallpapers`. If it detects that the internal `/app/src` directory has been overwritten by an incorrect volume mount, it will exit with a clear, user-friendly error message explaining the proper usage. This prevents confusing build failures.
...
- **Vercel Deployment & Caching**:
    - **Build Cache**: The project uses a custom build script (`scripts/vercel_build.sh`) to significantly speed up deployments. It manually restores cached assets (`webp` images, `lqip` placeholders, and the `gallery-data.json` metadata file) from a dedicated `.vercel/cache` directory (which Vercel persists between builds) before running the build. After the build, it saves the newly generated assets back to the cache directory. This ensures the generation script only processes new or changed wallpapers, saving significant time.
